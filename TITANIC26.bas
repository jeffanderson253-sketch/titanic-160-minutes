...
'   - Eight lifeboat GROUPS now honestly represent the twenty historical boats
'   - Wireless choices: broadcast, monitor replies, coordinate, or withdraw
'   - Signal reach and rescue readiness must both be developed
'   - Abandoning engineers or the wireless operator caps the command rating
...
Dim Shared TURN, SAVED, CREWLOST, WIRE, LISTEN, CONTACT, COORD
Dim Shared RKT, REACH, SIGPTS, RKTTHIS, WIREWD
...
Randomize Timer
Width 80, 50
_Title "Titanic: 160 Minutes - Version 2.6"
...
Do
    ...
    For TURN = 1 To 16
        ...
        If WIREWD = 0 Then AWO = TWO Else AWO = 0
        ...
        Do
            ...
            Select Case c
                ...
                Case 8
                    ShowManual
                Case 9
                    ' Endgame guard: people sitting in lifeboat groups never lowered
                    w = 0
                    For b = 1 To 8
                    ...
                    If TURN >= 15 And w > 0 Then
                        Color 12: Print "  *** "; LTrim$(Str$(w)); " souls sit in groups still hanging at the davits! ***"
                        warn = 1
                    End If
                    If TURN = 16 And ENWD = 0 Then
                        Color 12: Print "  *** "; TEN; " surviving engineers will be abandoned below! ***"
                        warn = 1
                    End If
                    If TURN = 16 And WIREWD = 0 Then
                        Color 12: Print "  *** The wireless operator will be abandoned at his station! ***"
                        warn = 1
                    End If
                    Color 7
                    ...
                    If warn = 1 Then
                        ...
                        Else
                            SetLast "Confirm orders", "Turn held open: launch groups or withdraw operational crew.", 14
                        End If
                    Else
                        done = 1
                    End If
...
Sub InitGame
    ...
    PWR = 1: DOC = 0: SAVED = 0: WIRE = 0: LISTEN = 0
    CONTACT = 0: COORD = 0: REACH = 0: SIGPTS = 0: CREWLOST = 0
    ENWD = 0: WIREWD = 0: URG = 0: PWRFAILT = 0: RKT = 0
End Sub
...
Sub CommandPanel
    ...
    s$ = "Groups covered " + LTrim$(Str$(CountBoats(0))) + "   Ready "
    s$ = s$ + LTrim$(Str$(CountBoats(1))) + "   Away "
    s$ = s$ + LTrim$(Str$(CountBoats(2))) + "   Loaded in davits "
    s$ = s$ + LTrim$(Str$(CountDavit))
    BoxKV "EVAC", s$, 11

    s$ = "Reach " + LTrim$(Str$(REACH)) + "/100   " + ContactName$
    s$ = s$ + "   Rescue readiness " + LTrim$(Str$(SIGPTS)) + "/200"
    BoxKV "SIGNALS", s$, 11

    s$ = "Pumps " + LTrim$(Str$(PUMPN))
    If PWRM = 1 Then s$ = s$ + "   Dynamos TENDED" Else s$ = s$ + "   Dynamos idle"
    If WIREWD = 1 Then
        s$ = s$ + "   Operator WITHDRAWN"
    ElseIf PWR = 0 Then
        s$ = s$ + "   Wireless DEAD"
    ElseIf AWO = 0 Then
        s$ = s$ + "   Wireless SENT"
...
Sub ShowAlerts
    ...
    ElseIf TURN >= 12 And CountBoats(0) > 0 Then
        Print "  ! "; CountBoats(0); " lifeboat groups remain covered with the bow going under."
        n = n + 1
    End If

    If n < 2 And PWR = 1 And AWO = 1 And WIREWD = 0 Then
        Color 14
        Print "  ! Wireless operator idle; signal reach or rescue coordination can improve."
        n = n + 1
    End If

    If n < 2 And TURN >= 14 And ENWD = 0 Then
        Color 12: Print "  ! Engineers remain below; leaving them there caps the final rating."
        n = n + 1
    End If

    If n < 2 And TURN >= 15 And WIREWD = 0 Then
        Color 12: Print "  ! Wireless operator still at his station; withdraw him before the end."
        n = n + 1
    End If
...
Sub ShowMainMenu
    ...
    Print "   3) DECKHAND ORDERS ";: Color 8: Print "prepare/launch groups, rockets": Color 15
    Print "   4) OFFICER ORDERS  ";: Color 8: Print "load groups, control crowds, rockets": Color 15
    ...
    Print "   7) WIRELESS ORDERS ";: Color 8: Print "broadcast, monitor, coordinate, withdraw": Color 15
...
Sub CrewScreen (t$)
    ...
    If WIREWD = 1 Then s$ = s$ + "   OPERATOR WITHDRAWN"
    BoxWrap "", s$, 10
    ...
    s$ = "Reach " + LTrim$(Str$(REACH)) + "/100   " + ContactName$
    s$ = s$ + "   Rescue " + LTrim$(Str$(SIGPTS)) + "/200"
    BoxKV "SIGNALS", s$, 11
...
Sub MenuDeckhands
    ...
    Color 7: Print "  Covered groups: "; CountBoats(0); "   Prepared: "; CountBoats(1);
    ...
    Print
    Color 15: Print "  1) Prepare a group      ";: Color 8: Print "COST 2 deckhands; ready several lifeboats": Color 15
    Print "  2) Launch a group       ";: Color 8: Print "COST 2 deckhands; only then are occupants safe": Color 15
    Print "  3) Fire rocket volley   ";: Color 8: Print "COST 1 deckhand; signal reach +5; once per turn": Color 15
...
Sub MenuOfficers
    ...
    Color 7: Print "  Ready groups: "; CountBoats(1); "   Port panic: "; ZK(4); "/5   Starboard panic: "; ZK(5); "/5"
    If URG = 0 Then Color 14: Print "  First rocket volley also adds +15 loading urgency and +1 panic on both decks."
    Print
    Color 15: Print "  1) Load a boat group    ";: Color 8: Print "COST 1 officer; optional steward assistance": Color 15
    Print "  2) Restore crowd order  ";: Color 8: Print "COST 1 officer; panic -2 on one deck": Color 15
    Print "  3) Fire rocket volley   ";: Color 8: Print "COST 1 officer; signal reach +5; once per turn": Color 15
    Print "  0) Return to command board"
    Color 7
...
Sub MenuEngineers
    ...
    End If
    If ENWD = 0 Then
        Color 12: Print "  "; TEN; " surviving engineers are still below and will be abandoned at 2:20."
    End If
    Print
    ...
    Print "  3) Withdraw engineers   ";: Color 8: Print "ENDS future work; saves every surviving engineer": Color 15
    Print "  0) Return to command board"
    Color 7
    ...
    Select Case c
        ...
        Case 3
            ...
            If y = 1 Then
                ENWD = 1: AEN = 0
                SetLast "Withdraw engineers", LTrim$(Str$(TEN)) + " engineers saved; all engineering work ends.", 14
            End If
    End Select
End Sub
...
Sub MenuWireless
    ...
    bgain = 18 - Int((TURN - 1) / 3)
    If bgain < 8 Then bgain = 8
    If WIRE = 0 Then bgain = bgain + 20

    cgain = 30 - 4 * COORD
    If cgain < 10 Then cgain = 10

    ceiling = REACH * 2
    If ceiling > 200 Then ceiling = 200

    Color 7: Print "  Signal reach: "; REACH; "/100   Contact: "; ContactName$
    Print "  Rescue readiness: "; SIGPTS; "/200   Present rescue ceiling: "; ceiling
    Print "  Broadcasts: "; WIRE; "   Monitoring turns: "; LISTEN; "   Coordinated turns: "; COORD

    If WIREWD = 1 Then
        Color 14: Print "  The operator has withdrawn and is safe. No further wireless orders."
        Color 7
        c = AskNum("Return to command board", 0, 0)
        Exit Sub
    End If

    Print
    Color 15: Print "  1) Broadcast CQD / SOS  ";: Color 8: Print "COST 1 operator; signal reach +"; bgain

    Color 15: Print "  2) Monitor and answer   ";
    Color 8
    If CONTACT = 0 And REACH < 20 Then
        Print "UNAVAILABLE until a distress call has reach 20"
    ElseIf CONTACT = 0 Then
        Print "COST 1; establish replies; readiness +15"
    ElseIf CONTACT = 1 And REACH < 45 Then
        Print "COST 1; weak replies; readiness +5"
    ElseIf CONTACT = 1 Then
        Print "COST 1; confirm Carpathia; readiness +35"
    Else
        Print "COST 1; receive rescue traffic; readiness +8"
    End If

    Color 15: Print "  3) Coordinate rescue    ";
    Color 8
    If CONTACT < 2 Then
        Print "UNAVAILABLE until Carpathia is confirmed"
    Else
        Print "COST 1; ice/boat-position updates; readiness +"; cgain
    End If

    Color 15: Print "  4) Withdraw operator    ";: Color 8: Print "PERMANENT; saves him; ends all wireless work"
    Color 15: Print "  0) Return to command board"
    Color 7

    c = AskNum("Wireless order", 0, 4)
    If c = 0 Then Exit Sub

    If c >= 1 And c <= 3 Then
        If AWO < 1 Then
            SetLast "Wireless order", "The operator has already worked this turn.", 14
            Exit Sub
        End If
        If PWR = 0 Then
            SetLast "Wireless order", "No power: only withdrawal remains possible.", 12
            Exit Sub
        End If
    End If
    ...
    Select Case c
        Case 1
            AWO = 0: WIRE = WIRE + 1
            REACH = REACH + bgain
            If REACH > 100 Then REACH = 100
            SetLast "Broadcast CQD / SOS", "Signal reach +" + LTrim$(Str$(bgain)) + "; now " + LTrim$(Str$(REACH)) + "/100.", 10

        Case 2
            If CONTACT = 0 And REACH < 20 Then
                SetLast "Monitor replies", "No distress signal has carried far enough; operator kept free.", 14
                Exit Sub
            End If

            AWO = 0: LISTEN = LISTEN + 1
            If CONTACT = 0 Then
                CONTACT = 1: mgain = 15
                out$ = "Ship replies heard; rescue readiness +15."
            ElseIf CONTACT = 1 And REACH >= 45 Then
                CONTACT = 2: mgain = 35
                out$ = "Carpathia confirms: 'Coming hard'; readiness +35."
            ElseIf CONTACT = 1 Then
                mgain = 5
                out$ = "Replies remain weak; broadcast farther before monitoring again."
            Else
                mgain = 8
                out$ = "Latest rescue traffic received; readiness +8."
            End If
            SIGPTS = SIGPTS + mgain
            If SIGPTS > 200 Then SIGPTS = 200
            SetLast "Monitor and answer", out$, 11

        Case 3
            If CONTACT < 2 Then
                SetLast "Coordinate rescue", "Carpathia is not yet confirmed; operator kept free.", 14
                Exit Sub
            End If
            AWO = 0: COORD = COORD + 1
            SIGPTS = SIGPTS + cgain
            If SIGPTS > 200 Then SIGPTS = 200
            SetLast "Coordinate rescue", "Ice and boat positions sent; readiness +" + LTrim$(Str$(cgain)) + ".", 10

        Case 4
            If AWO < 1 Then
                SetLast "Withdraw operator", "He cannot leave after receiving another order this turn.", 12
                Exit Sub
            End If
            y = AskNum("Withdraw for good? 1) Yes  2) No", 1, 2)
            If y = 1 Then
                WIREWD = 1: AWO = 0
                SetLast "Withdraw operator", "The operator leaves the wireless room alive; radio work ends.", 14
            End If
    End Select
End Sub
...
Sub ShowStatus
    ...
    Color 10: Print "  Saved in launched groups: "; SAVED; Tab(45); "Broadcasts: "; WIRE
    Color 11: Print "  Signal reach: "; REACH; "/100   Rescue: "; SIGPTS; "/200"; Tab(55); ContactName$
    ...
    If ENWD = 1 Then
        Color 10: Print "  Engineers withdrawn and counted safe."
    Else
        Color 12: Print "  Engineers still below; survivors will be lost unless withdrawn."
    End If

    If WIREWD = 1 Then
        Color 10: Print "  Wireless operator withdrawn and counted safe."
    Else
        Color 12: Print "  Wireless operator remains at his station."
    End If
...
Sub PrepareBoat
    If ADH < 2 Then
        SetLast "Prepare group", "Two free deckhands are required.", 12
        Exit Sub
    End If

    b = PickBoat(0, "Prepare which group")
    If b <= 0 Then
        If b = -1 Then SetLast "Prepare group", "No covered lifeboat groups remain.", 14
        Exit Sub
    End If

    ADH = ADH - 2
    BST(b) = 1
    SetLast "Prepare " + BN$(b), "Boats uncovered, plugs checked and swung outboard.", 10
End Sub

Sub LaunchBoat
    If ADH < 2 Then
        SetLast "Launch group", "Two free deckhands are required.", 12
        Exit Sub
    End If

    b = PickBoat(1, "Launch which group")
    If b <= 0 Then
        If b = -1 Then SetLast "Launch group", "No prepared lifeboat groups are at the davits.", 14
        Exit Sub
    End If
    ...
    If BLD(b) < 40 Then
        ...
        If y = 2 Then
            SetLast "Launch " + BN$(b), "Launch cancelled; the group remains in its davits.", 8
            Exit Sub
        End If
    End If
...
Sub LoadBoat
    If AOF < 1 Then
        SetLast "Load group", "No officer is free to supervise boarding.", 12
        Exit Sub
    End If

    b = PickBoat(1, "Load which group")
    If b <= 0 Then
        If b = -1 Then SetLast "Load group", "No prepared lifeboat groups are available.", 14
        Exit Sub
    End If

    If BLD(b) >= 150 Then
        SetLast "Load " + BN$(b), "That lifeboat group is already at combined capacity.", 14
        Exit Sub
    End If
    ...
    If AST > 0 Then
        ...
        For i = 0 To AST
            pv = CalcLoad(b, i)
            Print "    "; i; " steward(s) used  ->  boards "; pv; "  ->  group reaches "; BLD(b) + pv; "/150";
            If i > 0 Then
    ...
    ' Do not consume stewards who would add no one because the group or deck
    ...
    L = CalcLoad(b, sw)
    If L <= 0 Then
        SetLast "Load " + BN$(b), "The crowd is in chaos; no one reaches the group.", 12
        Exit Sub
    End If
...
Sub FireRockets (who)
    ...
    If URG = 0 Then
        ...
        SetLast "Fire first rocket volley", n$ + " reveals the emergency; reach +5, panic +1.", 14
    Else
        SetLast "Fire rocket volley", "Lookouts may see the white bursts; signal reach +5.", 10
    End If
End Sub
...
Sub ResolveTurn
    ...
    If TURN >= 13 Then
        ...
        If w > 0 Then
            Color 12: Print "  *** "; w; " souls are sitting in groups not yet lowered! ***"
            Color 7
        End If
    End If
    ...
    Color 7: Print "   Reach: "; REACH; "/100   Rescue: "; SIGPTS; "/200   Panic P:"; ZK(4); " S:"; ZK(5)
    Color 7
End Sub
...
Sub FinalReport
    ...
    ceiling = REACH * 2
    If ceiling > 200 Then ceiling = 200
    If resc > ceiling Then resc = ceiling
    ...
    Color 7: Print "    Saved in lifeboat groups:     "; SAVED
    Color 7: Print "    Pulled alive from the sea:    "; resc;
    Color 8: Print "  (reach ceiling "; ceiling; "; readiness "; SIGPTS; ")"
    Print
    ...
    If unl > 0 Then Color 12: Print "    In groups NEVER LAUNCHED:          "; unl
    If trapped = 0 And below = 0 And deck = 0 And unl = 0 Then
        Color 10: Print "    Every soul who could reach a lifeboat group was launched."
    End If
    If resc > 0 Then
    ...
    abandoned = 0
    If ENWD = 0 Then abandoned = abandoned + TEN
    If WIREWD = 0 Then abandoned = abandoned + 1
    crewfinal = CREWLOST + abandoned

    Color 8: Print "    Groups launched: "; nl; "/8   Seat use: "; util; "%   Rocket volleys: "; RKT
    Color 8: Print "    Wireless: "; WIRE; " broadcasts / "; LISTEN; " monitors / "; COORD; " coordinated"
    Color 8: Print "    Operational crew lost below: "; crewfinal; "   Deliberately left: "; abandoned
    ...
    capped = 0
    If abandoned > 0 And rating > 3 Then
        rating = 3
        capped = 1
    End If
    ...
    Select Case rating
        Case 0: Print "WORSE THAN HISTORY"
        Case 1: Print "A HARD-WON IMPROVEMENT"
        Case 2: Print "STRONG COMMAND"
        Case 3: Print "EXCELLENT COMMAND"
        Case 4: Print "MASTERFUL COMMAND"
        Case Else: Print "A NEAR-PERFECT EVACUATION"
    End Select
    Color 11: Print "  "; String$(74, Chr$(205))
    If capped = 1 Then
        Color 12: Print "    Rating capped: operational crew were knowingly left below."
    End If
    Color 7

    SectionHeader "COMMENDATIONS"
    awards = 0
    If nl = 8 Then Color 10: Print "    * ALL GROUPS AWAY";: Color 8: Print " - all twenty represented boats cleared": awards = awards + 1
    If util >= 90 And nl > 0 Then Color 10: Print "    * EVERY SEAT COUNTS";: Color 8: Print " - launched groups averaged 90%+ full": awards = awards + 1
    If PWRFAILT = 0 Then Color 10: Print "    * LIGHTS TO THE END";: Color 8: Print " - power held until the sinking": awards = awards + 1
    If CONTACT = 2 Then Color 10: Print "    * ANSWERED ON THE ETHER";: Color 8: Print " - firm contact made with Carpathia": awards = awards + 1
...
' Lists lifeboat groups in state 'want' (0=covered, 1=ready) and asks
' the player to choose one. Returns the group index, 0 for cancel,
' or -1 if none were eligible.
Function PickBoat (want, p$)
    n = 0
    Print
    If want = 0 Then
        Color 8: Print "  Covered groups:  ";
    Else
        Color 8: Print "  Ready groups at davits:  ";
    End If
...
Sub ShowManual
    ...
    SectionHeader "THE LIFEBOAT-GROUP PIPELINE"
    Color 7
    Print "   PREPARE (2 deckhands)  ->  LOAD (1 officer)  ->  LAUNCH (2 deckhands)"
    Print "   Eight groups represent Titanic's twenty boats. Each group has a rounded"
    Print "   combined capacity of 150. Loading a group does NOT save anyone -"
    Color 12: Print "   only a LAUNCHED group counts. Groups in the davits go down with her."
    Color 7

    SectionHeader "LOADING A LIFEBOAT GROUP"
    ...
    Print "   BROADCAST raises signal reach. MONITOR establishes replies and confirms"
    Print "   Carpathia. COORDINATE sends ice and boat positions, raising readiness."
    Print "   Rockets add 5 reach. Final water rescue is limited by readiness, by"
    Print "   twice signal reach, by 200, and by people who reach the water."

    SectionHeader "WITHDRAWAL"
    Color 12: Print "   Engineers and the wireless operator still at their posts at 2:20 are"
    Print "   counted abandoned. Withdrawal saves them but permanently ends their work."
    Print "   Knowingly abandoning operational crew caps the rating at EXCELLENT."
    ...
    Color 12: Print "   People in loaded groups are not saved until the group is launched."
...
Sub ShowSinkingShip
    ...
    ' one symbol on the water for every lifeboat group that got away
    n = 0
    For b = 1 To 8
...
Function ContactName$
    Select Case CONTACT
        Case 0: ContactName$ = "No replies"
        Case 1: ContactName$ = "Ship replies heard"
        Case Else: ContactName$ = "Carpathia confirmed"
    End Select
End Function
